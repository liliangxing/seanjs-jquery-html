<?php /*a:2:{s:54:"D:\download\ukcms\application\admin\view\user\add.html";i:1553153629;s:52:"D:\download\ukcms\application\admin\view\layout.html";i:1553153629;}*/ ?>
<!DOCTYPE html>
<html>
<head>
    <meta charset="utf-8">
...
<div class="panel">
    <div class="panel-heading">
        <div class="panel-title">添加后台账户</div>
    </div>
    <form class="panel-body" method="post" action="">
        <fieldset class="form-group">
            <label>角色</label>
            <select class="custom-select form-control" name="groupid">
                <?php if(is_array($glist) || $glist instanceof \think\Collection || $glist instanceof \think\Paginator): $i = 0; $__LIST__ = $glist;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?>
                  <option value="<?php echo htmlentities($vo['id']); ?>"><?php echo str_repeat('┝┈┈', $vo['deep']-1); ?><?php echo htmlentities($vo['names']); ?></option>
                <?php endforeach; endif; else: echo "" ;endif; ?>
            </select>
        </fieldset>
        <fieldset class="form-group">
            <label>账户*</label>
            <input type="text" class="form-control" name="username" placeholder="登录账户" value="">
        </fieldset>
        <fieldset class="form-group">
            <label>密码*</label>
            <input type="password" class="form-control" name="password" placeholder="6-20位" value="">
        </fieldset>
        <fieldset class="form-group" >
            <label>确认密码*</label>
        ...
        <fieldset class="form-group">
            <label>手机号码</label>
            <input type="number" class="form-control" name="mobile" placeholder="手机号码" value="">
        </fieldset>
        <fieldset class="form-group">
            <label>邮箱</label>
